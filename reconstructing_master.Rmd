...
```{r}
dp6 <- data_all %>% select(., c("hf_tourist_einheimisch","hf_einheit"))

dp6 <- dp6 %>% na.omit(.) %>%
  group_by(hf_tourist_einheimisch,hf_einheit) %>% summarise(occurences = n()) %>% mutate(
    percent = round((occurences/sum(occurences))*100,2)
  )

# renaming 
dp6$hf_tourist_einheimisch[dp6$hf_tourist_einheimisch == "ja"] <- "Einheimisch"
dp6$hf_tourist_einheimisch[dp6$hf_tourist_einheimisch == "nein"] <- "Tourist"

dp6$hf_einheit[dp6$hf_einheit == "ja"] <- "Einheit"
dp6$hf_einheit[dp6$hf_einheit == "nein"] <- "keine Einheit"


p6 <- ggplot(data = dp6, aes(x = hf_einheit, y = percent, fill = hf_tourist_einheimisch)) +
  geom_col(position = "dodge", alpha = 0.8) + xlab(" ") + ylab("Prozent %") +
  geom_text(aes(label = percent),
            position=position_dodge(width=0.9), size = 3,vjust=-0.5) +
  ggtitle("Einheit oder nicht -- Tourist vs Einheimisch") + ylim(0,100) +
...
## Verkehr
```{r}
dp7 <- data_all %>% select(., c("hf_grenzverkehr","hf_tourist_einheimisch")) %>%
  na.omit(.) %>% group_by(hf_grenzverkehr, hf_tourist_einheimisch) %>% summarise(occurences = n()) %>%
  mutate(percent = round((occurences/521)*100,2))

# renaming 
dp7$hf_tourist_einheimisch[dp7$hf_tourist_einheimisch == "ja"] <- "Einheimisch"
dp7$hf_tourist_einheimisch[dp7$hf_tourist_einheimisch == "nein"] <- "Tourist"


p7 <- ggplot(dp7, aes(x=hf_grenzverkehr, y=percent, fill = hf_tourist_einheimisch)) + 
  geom_col(position = "dodge", alpha = 0.8) + xlab("") + ylab("Prozent %") +
  geom_text(aes(label = percent),
            position=position_dodge(width=0.9), size = 3,vjust=-0.5) +
